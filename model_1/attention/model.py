...
class FeatureExtractor_resnet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.esa1 = ESA_blcok(dim = 256*256)
        # self.esa2 = ESA_blcok(dim = 128*128)
        # self.esa3 = ESA_blcok(dim = 64*64)
        # self.esa4 = ESA_blcok(dim = 32*32)
        # self.esaList = nn.ModuleList([self.esa1, self.esa2, self.esa3, self.esa4])
        dims = 256 * 256
        for i in range(self.layer_num):
            if i == 0:
                ...
                encoder_i = Down_resnet(
                    dims,in_channels = self.feature_dim,
...
class RSIS(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(self.layer_num):
            if i == 0:
                clstm_i = ConvLSTMCell(args, self.hidden_size * 2, self.hidden_size * 2, self.kernel_size,
                                       padding = padding)
            else:
                if i == self.layer_num - 1:
                    clstm_i = ConvLSTMCell(args, self.hidden_size * 2, self.hidden_size, self.kernel_size,
                                           padding = padding)
                else:
                    if i == self.layer_num - 2:
                        clstm_i = ConvLSTMCell(args, self.hidden_size * 4, self.hidden_size, self.kernel_size,
                                               padding = padding)
                    else:
                        clstm_i = ConvLSTMCell(args, self.hidden_size * 4, self.hidden_size * 2, self.kernel_size,
                                               padding = padding)
            self.clstm_list.append(clstm_i)
