...
class RSIS(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(self.layer_num - 1):
            decoder_i = Up_resnet(in_channels = self.feature_dim * 2,
                                  out_channels = self.feature_dim,
                                  baseWidth = args.baseWidth,
                                  init_feature = args.init_feature,
                                  scale = args.scale,
        ...
        self.clstm = ConvLSTMCell(args, self.feature_dim, self.feature_dim, self.kernel_size, padding = padding)
        ...
        self.conv_out = nn.Conv2d(args.init_feature, 1, self.kernel_size, padding = padding)
        ...
        for i in range(self.layer_num):
            if i == 0:
                if prev_hidden_temporal is None:
                    state = self.clstm(clstm_in, None)

                else:
                    state = self.clstm(clstm_in, prev_hidden_temporal)
                ...
                # print(hidden.shape)
                upsample_conv = self.up_list[i - 1]
                # print(upsample_conv)
                x = upsample_conv(x,feature_list[i])
...
def get_args_parser():
    ...
    parser.add_argument('--hidden_size', dest = 'hidden_size', default = 32, type = int)
